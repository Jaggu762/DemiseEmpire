    ...
    async execute(message, args, client) {
        if (args.length === 0) {
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: '📝 Usage', value: '`^imagegen <prompt>` - Generate image (uses seedream by default)\n`^imagegen flux <prompt>` - Use flux model\n`^imagegen seedream <prompt>` - Use seedream model', inline: false },
                    { name: '🎨 Available Models', value: '• **seedream** - Default model, fast image generation\n• **flux** - High quality image generation', inline: false },
        ...
        let model = 'seeddream'; // Changed default from 'flux' to 'seeddream'
        let prompt;
        ...
        if (firstArg === 'seedream') {
            ...
            // Changed: Use seedream as default instead of flux
            model = 'seedream';
            prompt = args.join(' ');
        }
...
async function generateAndSendImage(message, model, prompt) {
    const generatingMessage = await message.reply(`🔄 Generating image with ${model}... This may take 15-30 seconds.`);
    ...
    try {
        ...
        const errorEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '💡 Tips', value: '• Try a simpler prompt\n• Check if the API is working\n• Use different keywords\n• Try switching between models: `^imagegen flux <prompt>` or `^imagegen seedream <prompt>`', inline: false }
            )
